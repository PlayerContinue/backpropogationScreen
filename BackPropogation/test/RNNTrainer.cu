...
void RNNTrainer::train(thrust::device_vector<WEIGHT_TYPE> input, thrust::device_vector<WEIGHT_TYPE> output){
	...
	for (int i = 0; i < input.size(); i += this->_topology->InfoInLayer(0, TopologyBase::INPUT_NODES)){
		...
		this->applyGradiant();//Apply the gradiant
...
void RNNTrainer::findGradiant(thrust::device_vector<WEIGHT_TYPE>::iterator start_target){
	...
	auto find_output_error =
		...
		thrust::make_constant_iterator(this->_topology->InfoInLayer(0, TopologyBase::HIDDEN_NODES) + this->_topology->InfoInLayer(0, TopologyBase::INPUT_NODES))
		)//End tuple
		),//End zip
		functors::transform_functors::subtract<int>()
		)//End transform
		)//End permutation
		,
		this->layer_data.device_output_vector_begin
		...
		functors::transform_functors::subtract<WEIGHT_TYPE>()
		);//End Transform
		
	auto output_error_derivative = thrust::make_transform_iterator(
		thrust::make_zip_iterator(
		thrust::make_tuple(
		this->layer_data.device_output_vector_end - this->_topology->InfoInLayer(0, TopologyBase::OUTPUT_NODES),
		start_target
		)//End thrust
		),//End zip
	...
	//Multiply the current weights by the previous dy_k/dw_ij
	thrust::transform(
		this->layer_data.weight_vector_begin,
		this->layer_data.weight_vector_end,
		this->device_weight_error.begin(),
		this->device_weight_error.begin(),
		thrust::multiplies<WEIGHT_TYPE>()
		);

	//Add the output of the "to" node from time t to find the weight error at time t+1
	/*thrust::transform(
		thrust::make_zip_iterator(
		thrust::make_tuple(
		this->device_weight_error.begin(),
		thrust::make_permutation_iterator(
		this->device_input,
		this->layer_data.to_vector_begin
		),//End Permutation
		this->layer_data.from_vector_begin,
		this->layer_data.to_vector_begin
		)//End Tuple
		...
		) + (this->layer_data.from_vector_end - this->layer_data.from_vector_begin),
		this->device_weight_error.begin(),
		functors::transform_functors::find_weight_gradiant<WEIGHT_TYPE>()
		);*/
	
	
	// Permute the order of the error derivitive to match the required values
	auto permute_error_derivative_start = 
		thrust::make_permutation_iterator(
		output_error_derivative,
		this->layer_data.to_vector_end - this->_topology->InfoInLayer(0, TopologyBase::OUTPUT_WEIGHTS)
		);//End permutation

	//Multiply the weight error by the e'_k(t)
	thrust::transform(
		this->device_weight_error.end() - this->_topology->InfoInLayer(0, TopologyBase::OUTPUT_WEIGHTS),
		this->device_weight_error.end(),
		permute_error_derivative_start,
		this->device_weight_error.end() - this->_topology->InfoInLayer(0, TopologyBase::OUTPUT_WEIGHTS),
		thrust::multiplies<WEIGHT_TYPE>()
		);
	
	auto permute_output_error = thrust::make_permutation_iterator(
		find_output_error,
		this->layer_data.to_vector_end - this->_topology->InfoInLayer(0, TopologyBase::OUTPUT_WEIGHTS)
		);//End Permutation
