...
void LongTermShortTermNetwork::transferCellToGPU(unsigned int &start, unsigned int &number_node, unsigned int layer){
...
void LongTermShortTermNetwork::addCellToGPU(unsigned int start_new, unsigned int layer){
	if (layer < this->mBlocksLayers.size()){
		...
		int start_of_weights_to_insert_on = 0;
		int start_of_nodes_to_insert_on = 0;
		...
		//Add New Input Nodes
		start_of_weights_to_insert_on += this->number_weights_by_type[layer][INPUT_CELL];
		start_of_nodes_to_insert_on += this->number_nodes_by_type[layer][INPUT_CELL];
		for (unsigned int j = start_new; j < this->mBlocksLayers[layer].size(); j++){
			...
			this->GPUWeights.insert(weight_iterator, this->mBlocksLayers[layer][j].input_weights.begin(), this->mBlocksLayers[layer][j].input_weights.end());
			weight_iterator += this->mBlocksLayers[layer][j].input_weights.size();
			...
			this->GPUMapTo.insert(int_iterator,
				thrust::make_constant_iterator((int)start_of_weights_to_insert_on),
				thrust::make_constant_iterator((int)start_of_weights_to_insert_on)
			...
			start_of_weights_to_insert_on += this->mBlocksLayers[layer][j].input_weights.size() + this->mBlocksLayers[layer][j].number_memory_cells;
			start_of_nodes_to_insert_on += 1;
			this->numberOfWeightsInLayers[layer] += this->mBlocksLayers[layer][j].input_weights.size() + this->mBlocksLayers[layer][j].number_memory_cells;
			this->number_weights_by_type[layer][INPUT_CELL] += this->mBlocksLayers[layer][j].input_weights.size();
			this->number_nodes_by_type[layer][INPUT_CELL] += 1;
