			...
			if (!checkNeuronRemoved(this->v_layers.at(i).neurons[j])){//The current node has not been removed, use it
				...
				for (int k = 0; k < this->v_layers.at(i - 1).number_per_layer; k++){
					if (!checkNeuronRemoved(this->v_layers.at(i - 1).neurons[k])){//The neuron in the previous layer has not been removed, add it
						...
						sum += this->v_layers.at(i - 1).neurons[k].output*this->v_layers[i].neurons[j].weights[k];
					}
				}

				//Apply the bias
				sum += this->v_layers.at(i).neurons[j].bias;

				//Apply the sigmoid function
				this->v_layers.at(i).neurons[j].output = CNetwork::sigmoid(sum);

				//Possibly Temporary
				//States if the neuron has been activated
				if (isNeuronActivated(this->v_layers.at(i).neurons[j])){
					this->v_layers.at(i).neurons[j].activated += 1;
				}
			}
		}
	}
...
void CNetwork::backprop(double *in, double *tgt){
	...
	for (int i = 0; i < this->v_layers.at(this->v_num_layers - 1).number_per_layer; i++){
		...
		currentNeuron = &(this->v_layers.at(this->v_num_layers - 1).neurons[i]);
	...
	for (int i = this->v_num_layers - 2; i > 0; i--){
		for (int j = 0; j < this->v_layers.at(i).number_per_layer; j++){
			...
			for (int k = 0; k < this->v_layers.at(i + 1).number_per_layer; k++){
				currentNeuron = &(this->v_layers.at(i + 1).neurons[k]);
				if (!checkNeuronRemoved(*currentNeuron)){
					sum += currentNeuron->delta * currentNeuron->weights[j];
				}

			}
			currentNeuron = &(this->v_layers.at(i).neurons[j]);
	...
	if (this->alpha != 0){
		for (int layerPos = 1; layerPos < this->v_num_layers; layerPos++){
			for (int neuronPos = 0; neuronPos < this->v_layers.at(layerPos).number_per_layer; neuronPos++){
				currentNeuron = &(this->v_layers.at(layerPos).neurons[neuronPos]);
	...
	for (int layerNum = 1; layerNum < this->v_num_layers; layerNum++){
		for (int neuronPos = 0; neuronPos < this->v_layers.at(layerNum).number_per_layer; neuronPos++){
			if (!checkNeuronRemoved(this->v_layers.at(layerNum).neurons[neuronPos]) && !checkNeuronLocked(this->v_layers.at(layerNum).neurons[neuronPos])){//Check if Neuron is temp removed
				...
				for (int weightPos = 0; weightPos < this->v_layers.at(layerNum - 1).number_per_layer; weightPos++){
					...
					if (!checkNeuronRemoved(this->v_layers.at(layerNum - 1).neurons[weightPos])){

						//BETA * delta * output
						this->v_layers.at(layerNum).neurons[neuronPos].previousWeight[weightPos] =
							this->beta * this->v_layers.at(layerNum).neurons[neuronPos].delta * this->v_layers.at(layerNum - 1).neurons[weightPos].output;
						this->v_layers.at(layerNum).neurons[neuronPos].weights[weightPos] +=
							this->v_layers.at(layerNum).neurons[neuronPos].previousWeight[weightPos];
					}
				}

				this->v_layers.at(layerNum).neurons[neuronPos].previousBias = this->beta * this->v_layers.at(layerNum).neurons[neuronPos].delta;
				this->v_layers.at(layerNum).neurons[neuronPos].bias += this->v_layers.at(layerNum).neurons[neuronPos].previousBias;
			}
...
void CNetwork::removeNeuron(int layerPosition, int neuronPosition){
	...
	if (layerPosition >= this->v_num_layers || layerPosition < 0){//Layer doesn't exist
		...
		SNeuron &temp_neuron = this->v_layers.at(layerPosition).neurons[neuronPosition];//Retrive the current neuron
